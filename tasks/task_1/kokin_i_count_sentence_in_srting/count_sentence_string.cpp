...
std::vector<std::string> parseText(std::string str, int proc_num) {
    ...
    int splitSize = static_cast<int>(ceil(str.length()
        / static_cast<float>(proc_num)));
    ...
    for (int i = 1; i < splitCount + 1; i++) {
        int curreof = (i)*splitSize > str.length()
            ? str.length() : (i)*splitSize;
        while (curreof < static_cast<int>(str.length())
            && curreof < (i + 1) * splitSize &&
            (str[curreof] == '?' || str[curreof] == '!'
                || str[curreof] == '.')) {
            curreof++;
        }
